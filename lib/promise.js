        ...
        return new Promise((resolve, reject) => {
            ...
            */
            function handle(callback){
                try {
                    const result = callback(self.data)   //因为先promise先reslove了，此时成功返回的值，已经存进data里了
            ...
            if (self.status === PENDING) { //对应先执行then方法,然后才执行resolve,reject的情况。当前状态还是pending状态，将回调函数保存起来
                ...
                //回调函数异步执行
                setTimeout(() => {
                    handle(onResolved)  //主要用来改新返回的promise的状态
                })

            } else  { //rejected 对应前面的promise的状态先改变（先rejected）再去调用then方法的情况
                //回调函数异步执行
                setTimeout(() => {
                    handle(onRejected) 
                })
